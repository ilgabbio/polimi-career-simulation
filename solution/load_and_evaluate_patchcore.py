...
def save_pickle(data: Dict, dst_fpath: Path) -> None:
    dst_fpath.parent.mkdir(exist_ok=True, parents=True)

    with open(str(dst_fpath), "wb") as handle:
...
def run(methods, results_path, gpu, seed, save_segmentation_images):
    ...
    for dataloader_count, dataloaders in enumerate(dataloader_iter):
        ...
        with device_context:
            ...
            # Step 2: save predictions and GTs
            to_save = {}
            to_save["imagewise_pred"] = np.array(scores)
            to_save["imagewise_gt"] = np.array(anomaly_labels)

            image_names = [x[2] for x in dataloaders["testing"].dataset.data_to_iterate]
            to_save["fnames"] = np.array(image_names)
            to_save["pixelwise_pred"] = np.array(segmentations)
            to_save["pixelwise_gt"] = np.array(masks_gt)
            to_save["optimal_threshold"] = pixel_scores["optimal_threshold"]

            save_pickle(to_save, Path(results_path) / f"{dataset_name}.pkl")
